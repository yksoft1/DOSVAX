...
//      normal, shift , ctrl  , alt    ,kana  , kana_shift
	  ...
	  { 0x2960, 0x297e,   none, 0x29f0 ,0x29d1, 0x29a3 }, /* 29 `~ */ //29h `~Ñ£
...
//      normal, shift , ctrl  , alt    ,kana  , kana_shift
	  { 0x565c, 0x567c, 0x561c, none   ,0x56db, 0x56f0 }, /* 56 (102-key), \_ƒ| for AX */ //56h \|Û underscore
 ...
 */
	...
	case 0x1d:						/* Ctrl Pressed */
		if (INT16_AX_GetKBDBIOSMode() == 0x51 && (flags3 & 0x02)) {
			trimKana();//‰p”ƒJƒi keycode not assignged in AX
		}
		else {
			if (!(flags3 & 0x01)) {
				flags1 |= 0x04;
	...
	case 0x38:						/* Alt Pressed */
		...
		if (INT16_AX_GetKBDBIOSMode() == 0x51 && (flags3 & 0x02)) {// for AX: Is AX KBDBIOS in JP mode AND R-Alt pressed?
			if (!(flags1 & 0x0c))//neither Shift nor Ctrl pressed?
				add_key(0x3a00);//add Š¿Žš (Kanji) to key buffer
		}
		else
		{
			flags1 |= 0x08;
